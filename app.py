...
# This ensures the code finds the video folder even if 
# the launcher script changes the directory.
# -----------------------------------------------------------
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
VIDEO_DIR = os.path.join(BASE_DIR, "Exercise Videos")
...
def main():
    ...
    # Sidebar
    # Note: If Brand_Logo is not showing, you might need os.path.join(BASE_DIR, "Brand_Logo.jpg")
    st.sidebar.image("Brand_Logo.jpg", use_container_width=True) 
    st.sidebar.header("Settings")
    ...
    if source == "Pre-recorded Video":
        ...
        if video_name:
            # Check in the absolute directory path
            potential = os.path.join(VIDEO_DIR, video_name)
            
            if os.path.exists(potential):
                video_path = potential
            elif os.path.exists(video_name):
                 # In case user typed a full path manually
                video_path = video_name
            else:
                st.sidebar.error(f"Video not found in: {VIDEO_DIR}")
    ...
    if st.session_state.run:
        ...
        if source == "Live Webcam":
            ...
            if not video_path:
                st.error("Please enter a valid video path")
                # Reset run state so it doesn't hang
                st.session_state.run = False 
                return
            video_source = video_path
